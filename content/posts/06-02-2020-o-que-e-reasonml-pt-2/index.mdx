...
De início quero me desculpar as pessoas que me pediram muito essa segunda parte da série _"O que é ReasonML?"_.
...
Não é o objetivo deste artigo falar sobre ReasonML na plataforma nativa. Porém é impossível não 
...
_"React" entre aspas mesmo, pois não se trata do mesmo React conhecemos para web e sim uma implementação 
...
Para responder essa pergunta, vou precisar _"cutucar em algumas feridas"_ dos desenvolvedores JavaScript. Vejo uma certa resistência de 
...
comuns em aplicações JavaScript__ ,e o que se pode observar é que dos 10 erros listados é que: _7 deles são o que conhecemos por TypeErrors_.
...
*Recebeu um ticket de erro ou o reportaram que um erro ocorreu em produção e, para entender o problema e o que estava ocorrendo, abriu o console 
...
erro acontecendo no ambiente de produção ou até mesmo durante o desenvolvimento. Desenvolvedores JavaScript, estão sempre lidando com esse tipo 
de situação e o que começou como uma simples _"característica"_ da linguagem, em produção se torna uma dor de cabeça.
...
tipo *number* com *strings* ou tentar executar um *.map* em algo que não é um *array* e entre outras situações comuns para devs JavaScript_.
Por outro lado, existe um esforço em criar ferramentas que cuidem desses detalhes para você, como linters e supersets, como o TypeScript. 
Certamente utilizar essas ferramentas ou supersets, vão te ajudar a evitar a passar por esses erros de runtime. 

Alguns de vocês devem estar pensando agora: "*ah eu uso TypeScript, então eu já delego essas coisas para linguagem"*  e é verdade! TypeScript é uma 
opção para lidar com esses problemas. Porém, mesmo com TypeScript você pode acabar passando por alguns problemas semelhantes, já que TypeScript 
...
e sim, trazer a __sintaxe__ do JavaScript para uma linguagem robusta, madura e estáticamente tipada, que é *OCaml.*
Como eu havia dito na primeira parte do artigo, Reason é somente uma nova sintaxe, mais familiar a devs JavaScript, para a linguagem OCaml. 
...
verboso e rolou anotação de tipos por quase toda a parte do código. E se eu te dissesse que você pode desfrutar das vantagens de 
se utilizar tipos e ainda assim (quase) nunca ter que anota-los no seu código? Você não os vê, mas eles estão lá. Nos próximos trechos eu 
vou tentar apresentar um pouco de como funciona o sistema de tipos do ReasonML na prática.

## Tipos em ReasonML

Como eu disse anteriormente, o sistema de tipos de OCaml é incrível e muito poderoso, permite você modelar seu software de uma maneira muito 
clara e sem tirar a elegância da sintaxe. Como ReasonML é só uma sintaxe para OCaml, ele herda também o sistema de tipos. 
...
O primeiro ponto a se destacar no sistema de tipos de ReasonML é que paraticamente tudo é inferido, sim ***quase*** tudo. Isso significa que 
você apenas precisa __declarar__ seus tipos, mas não vai precisar __anota-los__ na hora de utilizar. 

Os exemplos de declaração de variáveis que dei  na primeira parte do artigo, com valores do tipo *string* e *int* não são um bom exemplo 
...
```
 ...
 Observe que eu não anotei nenhum tipo no segundo exemplo de Reason. No entanto, o compilador mesmo assim, sabe os tipos. 
...
preciso anota-los, o que não é uma realidade em TypeScript, caso eu queira restringir os valores a um tipo específico.
...
Como disse no começo desta seção, existem exceções quanto a inferência dos tipos em ReasonML. Records são um ótimo exemplo, já que poderiamos ter
...
```
O está acontecendo nessa função é o seguinte: A função recebe um parâmetro do tipo status e, utilizando um __switch__, que embora seja parecido,
...
seria o nome de cada pet. Para utilizar um variant como esse poderiamos fazer algo assim:
...
determinar que para _X_ possibilidade, retorne um valor _Y_. Isso é somente a forma mais básica de se utilizar pattern matching, poderiamos fazer algo mais granular ainda, veja:
```reason
...
```
Observe que na primeira possibilidade, ao invés de passar um nome de variável, eu passei uma string _"bob"_, isso significa que se o valor de __myPet__, for __Dog("bob")__,
...
Por isso se chama __pattern matching__, o intuito é encontrar um padrão e trata-lo.
...
ou None, que seria nada ou vázio. O exemplo abaixo mostra como podemos aplicar isso:
...
(o que não é, pois é um variant embutido na lingaugem) ele ficaria mais ou menos assim:
```reason
type option('a) =  
| Some('a)
| None;
...
```javascript
  ...
  but a pattern was expected which matches values of type
    string
```
Isso é o que garante que você está sempre trabalhando o tipo de dado correto. Se você em sua expressão, deixa explicito ao compilador, que você está trabalhando
