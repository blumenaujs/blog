...
provavelmente em algum momento você precisará utilizar o famigerado _any_ do TS, em suas aplicações.
...
tenta adicionar tipos ao JavaScript. Isso torna a adoção de TypeScript muito fácil aos desenvolvedores JS. Porém, por ser um superset, ele possui
...
não pode confiar 100% no seu sistema de tipos, o que é péssimo, já que a confiabilidade e a garantia de tipos, é a maior vantagem de se utilizar linguagens estaticamente
tipadas.
...
as vantagens que Reason tem em relação a ele. Algumas coisas como pattern matching e variants, irei abordar de forma superficial. Porém, vou deixar
...
Nesse exemplo, onde não há anotação de tipo para __someStudent__, o compilador do TS não consegue identificar que __someStudent__ é do tipo __Student__, portanto nos
...
Em contraste com TypeScript existe uma diferença crucial a ser mencionada. Lists e arrays, só podem armazenar elementos do mesmo __tipo__. Em TypeScript, encontramos essa flexibilidade, como podemos ver nos exemplos abaixo:
...
[currying](https://reasonml.github.io/docs/en/function#currying) e [argumentos nomeados](https://reasonml.github.io/docs/en/function#labeled-arguments), que irei abordar nos próximos trechos.
...
let message = switch somePerson {
  | { name: "Marcos", age } when age < 25 => "Marcos tem menos que 22 anos"
  | { name, age } => name ++ " : " ++ string_of_int(age)
};
```
...
como pattern matching possui. Para se chegar a um resultado semelhante em linguagens como TS ou JS, é necessário combinar switch e condicionais com if, o que pode e provavelmente vai ser péssimo para legibilidade.
...
A primeira vista, variants podem ser confundidas como as enums presentes em TypeScript, mas não se engane, apesar da semelhança, elas são muito mais que isso
...
Veja alguns exemplos de como seriam _"variants as symbols"_:
...
Já a segunda forma, é muito mais interessante, pois podemos utilizar variants que _"seguram"_ um ou mais valores.
...
Assim como no exemplo anterior, eu defini __2 constructors__ para meu type __pet__, sendo eles  __Dog__ e __Cat__. No entanto, neste exemplo, esses variants funcionam não como
...
const someDog: Pet = { type: PetEnum.DOG, name: "billy" }
const bob: Pet = { type: PetEnum.DOG, name: "bob" }
const dogWithoutName: Pet = { type: PetEnum.POG } // Linha A
...
_Antes que alguém critique este último exemplo, afirmando que não fiz uso de union types e types para os tipos __Dog__ e __Cat__, [aqui está um exemplo](https://www.typescriptlang.org/play/?ssl=1&ssc=1&pln=35&pc=2#) de como ficaria a mesma implementação fazendo uso dessas features.
...
Já em ReasonML, isso fica muito simples. Podemos combinar os variants e pattern matching, para modelarmos a nossa função, de forma que nenhum caso fique para trás e que não corramos o risco de obter saídas inesperadas.
...
```
Veja, como isso é idiomático na linguagem. Não precisei adicionar nenhum condicional, apenas determinei quais padrões existem e quais saídas elas teriam. Também é impossível que haja um __Dog__ sem name, pois só é possível 
utilizar o constructor __Dog__, se passado uma __string__ como o valor a ser "segurado".
...
É possível utilizar variants que "segurem" outros tipos de dados como __records__ ou __listas__. Assim como na seção anterior, não adicionei mais casos de uso, para não deixar ainda maior a seção.
Caso você se interesse mais por variants em Reason, vou deixar também um material adicional sobre isso no final do artigo.
...
Você possui uma função que faz um requisição http e retorna um __record__ do tipo __person__. No entanto você não tem certeza de que o retorno da API vai ser sempre um JSON,
...
a biblioteca [fp-ts](https://github.com/gcanti/fp-ts). Mas já adianto que nenhuma lib ou implementação para TS ou JS, chegará a resultados tão benéficios como os que vimos acima.
...
- [Handling Optional Values](https://medium.com/astrocoders/handling-optional-values-in-reasonml-with-belt-option-7e907819074b) - Artigo muito bacana do Gabriel Rubens, sobre como utilizar a stdlib de Reason para trabalhar options
